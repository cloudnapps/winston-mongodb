 ...
 * @param {Object} query Pass in query to find with.
 * @param {Object} options Stream options for this instance.
 * @param {Stream} stream Pass in a pre-existing stream.
 * @return {Stream}
 */
MongoDB.prototype.stream = function(query, options, stream) {
  options = options || {};
  ...
  if (start != null) {
    this.logDb.collection(this.collection, function(err, col) {
      ...
      col.find(query || {}, {skip: start}).toArray(function(err, docs) {
        if (err) {
          stream.emit('error', err);
          return;
        }
  ...
  this.logDb.collection(this.collection, function(err, col) {
    ...
    col.isCapped(function(err, capped) {
      ...
      if (capped) {
        var cursor = col.find(query, {}, {tailable: true, awaitData: true, numberOfRetries: Number.MAX_VALUE});
        stream.destroy = function() {
          this.destroyed = true;
          cursor.destroy();
        };
